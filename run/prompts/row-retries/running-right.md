Create Codex pet row `running-right` for `fushi`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Anime-style sea slug mascot named Fushi, preserving the body shape, face, palette, markings, and cute expression from fushi-settings.png. Compact whole-body sprite, readable at pet size, no text or scenery.. Preserve silhouette, face, palette, material, proportions, markings, and props.
